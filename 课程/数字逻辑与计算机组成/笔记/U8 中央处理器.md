...
- 数据通路由**操作元件（组合原逻辑元件）和存储元件（时序逻辑元件）**通过总线或分散方式连接而成的进行数据存储处理和传送的路径、
- 存储元件
  - 寄存器（组）
    ...
    - 一个写口（**时序逻辑**操作）：写使能为1的情况下，时钟边沿到来时，busW传来的值开始被写入RW指定的寄存器中。
  - 理想寄存器
...
- CPI
  ...
  - CPU执行时间=CPU时钟周期数/程序*时钟周期=指令条数/程序\***CPI**\*时钟周期
...
- <img src="https://thdlrt.oss-cn-beijing.aliyuncs.com/image-20231213005255595.png" alt="image-20231213005255595" style="zoom:33%;" />
  - 寄存器有32个32位寄存器（0号始终为0），寄存器编号占5位
    - <img src="https://thdlrt.oss-cn-beijing.aliyuncs.com/image-20231213010412336.png" alt="image-20231213010412336" style="zoom:33%;" />
...
- B-型指令的数据通路
  ...
  - 下地址逻辑
    - <img src="https://thdlrt.oss-cn-beijing.aliyuncs.com/image-20231213104859158.png" alt="image-20231213104859158" style="zoom:33%;" />
- J-型指令的数据通路
  - <img src="https://thdlrt.oss-cn-beijing.aliyuncs.com/image-20231213104911869.png" alt="image-20231213104911869" style="zoom: 50%;" />
  - <img src="https://thdlrt.oss-cn-beijing.aliyuncs.com/image-20231213105150643.png" alt="image-20231213105150643" style="zoom:33%;" />
- 完整数据通路
