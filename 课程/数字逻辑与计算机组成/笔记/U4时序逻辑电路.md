...
- 用1个或多个输入信号能**驱动双稳态元件进入稳定状态**，这些输入信号称为激励信号或激励输入。通常根据**不同的激励输入信号**来命名存储元件，如SR、JK、D、T 等不同的激励输入信号
...
- D触发器
  ...
  - 时延
    - 从时钟触发边沿到来，到输出端Q改变为Q的时间称为所存延迟，分为$t_{pLH}$和$t_{pHL}$
