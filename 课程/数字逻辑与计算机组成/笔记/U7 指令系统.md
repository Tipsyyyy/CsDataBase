...
- 偏移寻址方式
  ...
  - 相对寻址：指令地址码给出一个**偏移量**(带符号数)，基准地址R隐含由**PC**给出。
    - EA=(PC)+A
    - 实现jmp转移指令
...
- 定长操作码编码
  ...
  - <img src="https://thdlrt.oss-cn-beijing.aliyuncs.com/image-20231129131657863.png" alt="image-20231129131657863" style="zoom:33%;" />
    - RX：X为索引寄存器（其内容会被加到最终的内存地址中，用于变址）；B为基址寄存器；D为附加偏移量的立即数
- 扩展（变长）操作码编码
  - 设某指令系统指令字是16位，每个地址码为6位。若二地址指令15条，一地址指令34条，则剩下零地址指令最多有多少条？
  - 二地址指令中地址占据12位，故剩下4位作为操作码，即16种（实际使用0000~1110）
...
- 按指令格式的复杂度来分
  ...
  - 精简指令集计算机RISC
    ...
    - 以**RR**方式工作：除了loard/store之外只访问寄存器
    - 采用大量**通用寄存器**，以减少访存次数
