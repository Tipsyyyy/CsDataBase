...
- 低电平和高电平指的都是一个电压范围
- <img src="https://thdlrt.oss-cn-beijing.aliyuncs.com/image-20230915153444801.png" alt="image-20230915153444801" style="zoom:33%;" />
...
- 输入的范围大、容错好，输出的范围小、较为精确
  -  <img src="https://thdlrt.oss-cn-beijing.aliyuncs.com/image-20230915154049841.png" alt="image-20230915154049841" style="zoom:33%;" />
- <img src="https://thdlrt.oss-cn-beijing.aliyuncs.com/image-20230915154123780.png" alt="image-20230915154123780" style="zoom:50%;" />
- <img src="https://thdlrt.oss-cn-beijing.aliyuncs.com/image-20230915154325270.png" alt="image-20230915154325270" style="zoom:33%;" />

### CMOS晶体管

- MOS晶体管可被模型化为一种3端子压控电阻导体，将电压加到一个端子，来控制其他两个端子间的电阻。
  - <img src="https://thdlrt.oss-cn-beijing.aliyuncs.com/image-20230915154534355.png" alt="image-20230915154534355" style="zoom:33%;" />
  - 栅极和源级之间的电压差V~gs~控制源极和漏极间电阻R~ds~
...
- 传播延迟：从输入信号变化到**引起**输出信号**变化**所需的时间
  - t~pHL~：输入变化引起相应输出从高到低变化的时间
  - t~pLH~：输入变化引起相应输出从低到高变化的时间
...
- 对偶定律：
  - 对于原式Y，将`·`和`+`以及`0`和`1`互换，得到对偶式Y^D^
...
- 每个输入组合都有一个确定的输出值，n个变量有$2^{2^n}$种不同的逻辑函数
...
- 标准乘积和：每个逻辑变量出现且仅出现一次的乘积项，有$2^n$个
  ...
  - 标准乘积项也称为最小项，每个最小项对应真值表中一个输入组合，赋值该输入组合后，最小项的运算结果为1。记m~i~
  - 标准求和项也称为最大项，每个最大项对应真值表中一个输入组合，赋值该输入组合后，最大项的运算结果为0。记M~i~
  - i是输入值使结果特殊的二进制：<img src="C:\Users\caome\AppData\Roaming\Typora\typora-user-images\image-20230922140633054.png" alt="image-20230922140633054" style="zoom:33%;" />
...
- 实质蕴涵项：覆盖的最小项中至少有一个最小项是**没有被其他质蕴涵项**所覆盖的质蕴涵项。
